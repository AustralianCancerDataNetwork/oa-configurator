...
- **`configure` validates default references before saving.** A default such as `cdm_db = "cdm_db"` must name an existing entry even when the caller did not pass that field explicitly. Provision referenced connections, databases, providers, models, and vector stores first, use nested `--set` values to create them in the same non-interactive command, or use the interactive flow to create or select them. An invalid candidate leaves the existing file unchanged.
