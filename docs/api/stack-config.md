...
`unresolved_refs` is also exported directly from `oa_configurator`; callers do
not need to import the implementation module.
...
`mismatched_kind_refs` is also exported directly from `oa_configurator`.
