...
A schema problem raises `PackageConfigInvalidError`. Use `tool_name` to identify the affected `[tools.<name>]` section and `errors()` to attach messages to form fields, including nested fields and model-level errors. These details deliberately omit rejected values and validator context, and the exception does not retain the original pydantic error, so displaying or logging the exception cannot echo a submitted secret. A missing or wrong-kind reference raises `ConfigurationError` with the package field path.
...
Planning never reads or writes the active configuration file. The returned candidate keeps `loaded_path` as provenance, but your application still decides whether and when to call `save_stack_config()`. Failures are ordinary `ConfigurationError` or `PackageConfigInvalidError` exceptions: this API does not print CLI guidance or raise `typer.Exit`.

::: oa_configurator.package_base.PackageConfigBase

::: oa_configurator.package_base.PackageConfigInvalidError

## Errors from the stack file itself

`StackConfigInvalidError` is the whole-file sibling of the above, raised by [`load_stack_config()`](loader.md) when a file parses as TOML but does not validate as a `StackConfig`. It sanitizes identically by providing field paths and reasons, never the rejected value, because a stack file holds every password and API key in the deployment.

::: oa_configurator.package_base.StackConfigInvalidError
